...
(
	set -x
	debootstrap --no-check-gpg --arch=armhf --verbose --variant='minbase' --include='apt-utils,iproute,iputils-ping,locales,vim' jessie "$rootfsDir" "http://mirrors.aliyun.com/raspbian/raspbian/ jessie main contrib non-free rpi"
)
...
# shrink a little, since apt makes us cache-fat (jessie: ~157.5MB vs ~120MB)
...
if [ -d "$rootfsDir/etc/apt/apt.conf.d" ]; then
	...
	# "debian:jessie" base image was "29.88 MB", where with this it was only
	# "8.273 MB".
